...
import { MUTATION_CREATE_UPDATE_PRODUCT, MUTATION_FILE_CHAT_UPLOAD } from "utils/Mutations/Administrative";
...
const MessagesScreen: React.FC<MessagesScreenProps> = ({
  ...
  const [UpdateCreateFileChat] = useMutation(MUTATION_FILE_CHAT_UPLOAD);
  ...
  const sendFileMessage = async (fileMessage: MessageDataObjType) => {
    try {
    ...
    const images = await fileMessage?.file?.map(file => ({
      TIPO_DOC: 'A',
      FILE: file
    }));
    ...
    const ResponseFile = await UpdateCreateFileChat({
      variables: {
        condition: {
          ESTADO: 'A',
          FILESUPLOAD: images,
